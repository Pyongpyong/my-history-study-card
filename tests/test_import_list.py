...
def test_import_and_list(client: TestClient):
    ...
    expected_keywords = SAMPLE_PAYLOAD["keywords"] + SAMPLE_PAYLOAD["tags"]
    assert detail["keywords"] == expected_keywords
    assert detail["timeline"] == SAMPLE_PAYLOAD["timeline"]
    assert detail["categories"] == SAMPLE_PAYLOAD["categories"]
    assert detail["eras"] == SAMPLE_PAYLOAD["eras"]
